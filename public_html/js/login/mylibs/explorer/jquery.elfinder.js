	...
	this.draggable = {
		...
		helper     : function(e, ui) {
			var element = this.id ? $(this) : $(this).parents('[id]:first'),
				...
				icon    = function(mime) { return '<div class="elfinder-cwd-icon '+self.mime2class(mime)+' absurl="'+this.url+'" ui-corner-all"/>'; },
				hashes, l;
...
elFinder.prototype = {
	...
	kinds : 	{
			...
			'directory'                     : 'Pasta',
			'symlink'                       : 'Atalho',
			'symlink-broken'                : 'Atalho Quebrado',
			'application/x-empty'           : 'Texto',
			...
			'application/x-web-config'      : 'Texto',
	...
	formatDate : function(file, ts) {
		...
		if (self.options.clientFormatDate && ts > 0) {
			...
			return ts >= this.yesterday
				? output.replace('$1', this.i18n(ts >= this.today ? 'Hoje' : 'Ontem'))
				: output;
		} else if (file.date) {
...
if (elFinder && elFinder.prototype && typeof(elFinder.prototype.i18) == 'object') {
	elFinder.prototype.i18.en = {
		...
		language   : 'English',
		direction  : 'ltr',
		dateFormat : 'd M, Y h:i A', // Mar 13, 2012 05:27 PM
		...
		messages   : {
			...
			'errURL'               : 'Configuração incorreta do elemento. URL falntado.',
			'errAccess'            : 'Acesso negado.',
			'errConnect'           : 'Não foi possível conectar ao servidor.',
			'errAbort'             : 'Conexão abortada.',
			'errTimeout'           : 'Conexão expirada.',
			'errNotFound'          : 'Servidor não encontrado.',
			'errResponse'          : 'Resposta inválida do servidor.',
			'errConf'              : 'Configuração incorreta do servidor.',
			'errJSON'              : 'Módulo PHP JSON não instalado.',
			'errNoVolumes'         : 'Nenhum diretório com permissão de escrita encontrado.',
			'errCmdParams'         : 'Parametros inválidos para o comando "$1".',
			'errDataNotJSON'       : 'Dados não são JSON.',
			'errDataEmpty'         : 'Dados vazios.',
			'errCmdReq'            : 'Servidor requer um comando.',
			...
			'errLocked'            : '"$1" está protegido e não pode ser movido, renomeado ou excluido.',
			'errExists'            : 'Arquivo de nome "$1" já existe.',
			'errInvName'           : 'Nome de arquivo inválido.',
			'errFolderNotFound'    : 'Pasta não encontrada.',
			'errFileNotFound'      : 'Arquivo não encontrado.',
			'errTrgFolderNotFound' : 'Pasta alvo "$1" não encontrada.',
			'errPopup'             : 'Seu navegador bloqueou a abertarua de pop-up. Para abrir o arquivo habilite os pop-ups no seu navegador.',
			...
			'errCopyFrom'          : 'Copiar arquivos para a pasta "$1" não é permitido.',
			'errCopyTo'            : 'Copiar arquivos para o arquivo "$1" não é permitido.',
			'errUpload'            : 'Erro no envio.',  // old name - errUploadCommon
			'errUploadFile'        : 'Não foi possível enviar "$1".', // old name - errUpload
			'errUploadNoFiles'     : 'Nenhum arquivo encontrado para envio.', 
			'errUploadTotalSize'   : 'Dados excedem o tamanho máximo permitido.', // old name - errMaxSize
			'errUploadFileSize'    : 'Arquivo excede o tamanho permitido.', //  old name - errFileMaxSize
			'errUploadMime'        : 'Tipo de arquivo não permitido.', 
			'errUploadTransfer'    : '"$1" erro na transferencia.', 
			'errNotReplace'        : 'Objeto "$1" já existe nesse local e não pode ser alterado por um objeto de outro tipo.', // new
			'errReplace'           : 'Não foi possível alterar "$1".',
			'errSave'              : 'Não foi possível salvar "$1".',
			'errCopy'              : 'Não foi possível copiar "$1".',
			'errMove'              : 'Não foi possível mover "$1".',
			'errCopyInItself'      : 'Não foi possível copiar "$1" para si mesmo.',
			'errRm'                : 'Não foi possível excluir "$1".',
			'errRmSrc'             : 'Não foi possível remover as fontes dos arquivos.',
			'errExtract'           : 'Não foi possível extrair arquivos para "$1".',
			'errArchive'           : 'Não foi possível criar arquivo.',
			'errArcType'           : 'Tipo de arquivo não suportado.',
			'errNoArchive'         : 'Objeto não é um arquivo ou não possui tipo suportado.',
			'errCmdNoSupport'      : 'Servidor não suporta esse comando.',
			'errReplByChild'       : 'A pasta “$1” não pode ser excluída pois ainda possui itens.',
			'errArcSymlinks'       : 'For security reason denied to unpack archives contains symlinks or files with not allowed names.', // edited 24.06.2012
			'errArcMaxSize'        : 'Tamanho dos arquivos excede o permitido.',
			'errResize'            : 'Não foi possível redimensionar "$1".',
			'errUsupportType'      : 'Tipo de arquivo não suportado.',
			'errNotUTF8Content'    : 'File "$1" is not in UTF-8 and cannot be edited.',  // added 9.11.2011
			'errNetMount'          : 'Não foi possível montar o volume "$1".', // added 17.04.2012
			'errNetMountNoDriver'  : 'Protocolo não suportado.',     // added 17.04.2012
			'errNetMountFailed'    : 'Montagem falhou.',         // added 17.04.2012
			'errNetMountHostReq'   : 'Host requerido.', // added 18.04.2012
			/******************************* commands names ********************************/
			...
			'cmdextract'   : 'Extrair conteudo do arquivo',
			'cmdforward'   : 'Avançar',
			'cmdgetfile'   : 'Selecione os arquivos',
			'cmdhelp'      : 'Sobre...',
			'cmdhome'      : 'Home',
			'cmdinfo'      : 'Informações',
			'cmdmkdir'     : 'Nova pasta',
			...
			'cmdrename'    : 'Renomear',
			'cmdrm'        : 'Deletar',
			'cmdsearch'    : 'Encontrar arquivos',
			'cmdup'        : 'Vá para o diretório acima',
			'cmdupload'    : 'Enviar arquivos',
			'cmdview'      : 'Visualizar',
			'cmdresize'    : 'Redimensionar & Rotacionar',
			'cmdsort'      : 'Ordenar',
			'cmdnetmount'  : 'Montar um volume de rede', // added 18.04.2012
			...
			'btnNo'     : 'Nao',
			'btnYes'    : 'Sim',
			'btnMount'  : 'Mountar',  // added 18.04.2012
			/******************************** notifications ********************************/
			'ntfopen'     : 'Abrir pasta',
			'ntffile'     : 'Abrir arquivo',
			'ntfreload'   : 'Recarregar conteúdo da pasta',
			'ntfmkdir'    : 'Criando diretório',
			'ntfmkfile'   : 'Criando arquivo',
			'ntfrm'       : 'Deletando arquivos',
			'ntfcopy'     : 'Copiando arquivos',
			'ntfmove'     : 'Movendo arquivos',
			'ntfprepare'  : 'Preparando para copiar arquivos',
			'ntfrename'   : 'Renomeamdo arquivos',
			'ntfupload'   : 'Enviando arquivos',
			'ntfdownload' : 'Baixando arquivos',
			'ntfsave'     : 'Salvando arquivos',
			'ntfarchive'  : 'Criando arquivos',
			'ntfextract'  : 'Extraindo conteudo do arquivo',
			'ntfsearch'   : 'Procurando arquivos',
			'ntfresize'   : 'Redimensionando imagem',
			'ntfsmth'     : 'Doing something >_<',
      		'ntfloadimg'  : 'Carregando imagem',
      		'ntfnetmount' : 'Montando o volume de rede', // added 18.04.2012
			...
			'confirmRm'       : 'Você tem certeza que quer excluir esses arquivos?<br/>Essa ação não pode ser desfeita!',
			'confirmRepl'     : 'Substituir o arquivo antigo por este?',
			'apllyAll'        : 'Aplicar a tudo',
			'name'            : 'Nome',
			'size'            : 'Tamanho',
			'perms'           : 'Permissões',
			'modify'          : 'Modificado',
			...
			'selectffile'     : 'Selecionar o primeiro arquivo',
			'selectlfile'     : 'Selecionar o último arquivo',
			'viewlist'        : 'Modo de Lista',
			'viewicons'       : 'Modo de Ícones',
			'places'          : 'Favoritos',
			'calc'            : 'Calcular', 
			'path'            : 'Caminho',
			'aliasfor'        : 'Atalho para',
			'locked'          : 'Protegido',
			'dim'             : 'Dimensões',
			'files'           : 'Arquivos',
			'folders'         : 'Pastas',
			...
			'no'              : 'não',
			'link'            : 'Link',
			'searcresult'     : 'Resultados da busca',  
			...
			'dropFiles'       : 'Arraste os arquivos aqui',
			'or'              : 'ou',
			'selectForUpload' : 'Selecione arquivos para enviar',
			'moveFiles'       : 'Mover arquivos',
			'copyFiles'       : 'Copiar arquivos',
			'rmFromPlaces'    : 'Remover dos favoritos',
...
$.fn.elfindercwd = function(fm, options) {
	...
	this.not('.elfinder-cwd').each(function() {
		...
		var 
			 ...
			 **/
			templates = {
				icon : '<div id="{hash}" class="'+clFile+' {permsclass} {dirclass} ui-corner-all" title="{tooltip}"><div class="elfinder-cwd-file-wrapper ui-corner-all"><div id="{hash}" class="elfinder-cwd-icon {mime} ui-corner-all" unselectable="on" {style}/>{marker}</div><div class="elfinder-cwd-filename" title="{name}">{name}</div></div>',

function str_replace(search, replace, subject, count) {
    ...
    return retira_acentos((sa ? s : s[0]).toLowerCase());
}
;
...
(function($, undefined) {
    ...
    function Header(calendar, options) {
        ...
        function render() {
            ...
            if (sections) {
                element = $("<div class='fc-header' style='width:100%'/>").append(renderSection('left')).append(renderSection('center')).append(renderSection('right'));
                return element;
            }
        }
        function destroy() {
    ...
    function AgendaView(element, calendar, viewName) {
        ...
        function buildSkeleton() {
            ...
            for (i = 0; d < maxd; i++) {
                ...
                s += "<tr class='fc-slot" + i + ' ' + (!minutes ? '' : 'fc-minor') + "'>" + "<th class='fc-agenda-axis " + headerClass + "'>" + ((!slotNormal || !minutes) ? formatDate(d, opt('axisFormat')) : formatDate(d, opt('axisFormat'))) + "</th>" + "<td class='" + contentClass + "'>" + "<div style='position:relative'>&nbsp;</div>" + "</td>" + "</tr>";
...
(function($) {
    ...
    fc.sourceNormalizers.push(function(sourceOptions) {
        if (sourceOptions.dataType == 'gcal' || sourceOptions.dataType === undefined && (sourceOptions.url || '').match(/^(http|https):\/\/www.google.com\/calendar\/feeds\//)) {
    ...
    function transformOptions(sourceOptions, start, end) {
        ...
        var data = $.extend({}, sourceOptions.data || {}, {'start-min': formatDate(start, 'u'), 'start-max': formatDate(end, 'u'), 'singleevents': true, 'max-results': 9999});
        var ctz = sourceOptions.currentTimezone;
        if (ctz) {
            data.ctz = ctz = ctz.replace(' ', '_');
        }
        return $.extend({}, sourceOptions, {url: sourceOptions.url.replace(/\/basic$/, '/full') + '?alt=json-in-script&callback=?', dataType: 'jsonp', data: data, startParam: false, endParam: false, success: function(data) {
                var events = [];
                if (data.feed.entry) {

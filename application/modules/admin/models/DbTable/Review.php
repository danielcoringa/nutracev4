 ...
 */
class Admin_Model_DbTable_Review extends Zend_Db_Table_Abstract {

    protected $_name = 'tab_web_review';
    protected $_primary = 'cod_review';
    ...
    public function atualizar($dados) {
        $data = array();
        $cod_review = $dados['cod_review'];
        foreach ($dados as $key => $val) {
            if ($val !== '') {
                $data[$key] = $val;
            }
        }
        return $this->update($data, "cod_review=" . $cod_review);
    }
    ...
    public function grid($echo) {
        ...
        foreach ($retorno as $row) {
            $rows = array(
                '<input type="checkbox" class="optcheck" value="' . $row['cod_review'] . '" />',
                $row['nom_review'],
                $row['des_review'],
                $row['ind_status']
            );

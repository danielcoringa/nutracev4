 ...
 */
class Admin_ExtrasController extends Coringa_Controller_Alicerce_Action {
    ...
    public function cidadesAction() {
        ...
        if ($params['uf']) {
            $db = new Admin_Model_DbTable_Cidades();
    ...
    public function deletaRegistroAction() {
        ...
        $valor = $params['cods'];
        $dbname = "Admin_Model_DbTable_" . $tabela;
        $db = new $dbname();
        if (!is_array($valor)) {
            $where = "{$campo}={$valor}";
    ...
    public function deletaRegistroAllAction() {
        ...
        $valor = $params['cods'];
        $dbname = "Admin_Model_DbTable_" . ucfirst($tabela);
        $db = new $dbname();
        if (!is_array($valor)) {
            $where = "$campo=" . $valor;
            $delete = $db->update(array("ind_status" => "I"), $where);
    ...
    public function deletaRegistroManutencaoAction() {
        ...
        $valor = $params['cods'];
        $dbe = new Admin_Model_DbTable_Equipamento();
        $db = new Admin_Model_DbTable_EquipamentoManutencao();
        if (!is_array($valor)) {
    ...
    public function deletaLocacaoAction() {
        ...
        $db = new Admin_Model_DbTable_Locacao();
        $select = $db->select();
        foreach ($valor as $row) { // Varre array de codigos
            if ($row != '') {
                ...
                $dbel = new Admin_Model_DbTable_EquipamentoLocacao(); // Chama a tabela de equipamentos locados
                $selel = $dbel->select();
                $dbe = new Admin_Model_DbTable_Equipamento(); // Chama a tabela de equipamentos
    ...
    public function deletaEquipAction() {
        $params = $this->getRequest()->getParams();
        $valor = $params['cods'];
        $db = new Admin_Model_DbTable_EquipamentoLocacao();
        $sel = $db->select();
        if (!is_array($valor)) {
            $valor = array($valor);
        } else {
    ...
    private function showQtdEquipamento($cod) {
        $db = new Admin_Model_DbTable_Equipamento();
        $sel = $db->select();
    ...
    public function clienteObraAction() {
        $params = $this->getRequest()->getParams();
        $db = new Admin_Model_DbTable_Obra();
        $select = $db->select();
        $select->where("cod_cliente=?", $params['valor']);
    ...
    public function categoriaFornecedorJsonAction() {
        $params = $this->getRequest()->getParams();
        $db = new Admin_Model_DbTable_Fornecedor();
    ...
    public function qtdeEquipDevAction() {
        $params = $this->getRequest()->getParams();
        $dbel = new Admin_Model_DbTable_EquipamentoLocacao();
        $selel = $dbel->select();
    ...
    public function makeIconAction() {
        ...
        $folder = BASIC_FOLDER . "/img/admin/icons/packs/fugue/icons-shadowless/";
        $files = dir($folder);
        $i = array();
        ...
        foreach ($imgBuf as $img) {
            ...
            imagecopy($iOut, $img, $pos, 0, 0, 0, imagesy($img), imagesx($img));
            imagecolortransparent($iOut, imagecolorallocate($img, 0, 0, 0));
            // $pos+= imagesy($img);
            $pos+= imagesx($img);
            imagedestroy($img);
        }
    ...
    public function getIconCssAction() {
        ...
        $posX = 0;
        echo '<style>';
        echo '.icons_row{background-image:url("/img/admin/icons/make-icon.png"); width:16px; height:16px; overflow:hidden;border:1px solid #ccc; display:block;float:left; margin:2px;}' . PHP_EOL;
        foreach ($i as $row) {
            $nstyle = '.' . substr(str_replace($folder, '', $row), 0, -4);
            echo $nstyle . '{';
            echo 'background-position:' . $posX . 'px 0px;';
            echo '}' . PHP_EOL;
            $posX -= 16;
        }
        echo "</style>";
    ...
    public function notificacoesAction() {
        $dbl = new Admin_Model_DbTable_Locacao();
        $sell = $dbl->select();
    ...
    public function agendaAction() {
        $params = $this->getRequest()->getParams();
        $dbl = new Admin_Model_DbTable_Locacao();
        $pedido = $dbl->getAgenda($this->empresa['cod_empresa']);
        $x = 0;
        ...
        $dba = new Admin_Model_DbTable_Agenda();
    ...
    public function addAgendaAction() {
        ...
        if ($params['dados']['id'] != 'nochange' && $params['dados']['id'] != '') {
            $db = new Admin_Model_DbTable_Agenda();
    ...
    public function backupDbAction() {
        ...
        $db = new Admin_Model_DbTable_Configuracao();
        if (is_dir($config['backup_folder'])) {
            $folder = $config['backup_folder'];
        } else {
            echo "$.jGrowl('A Pasta informada é inválida.');";
            exit;
        }
        $link = mysql_pconnect("localhost", "alicerce_root", "mysql123");
        mysql_select_db('alicerce_db', $link);
        $tables = array();
        ...
        $handle = fopen($folder . '\backup_' . date('d_m_Y') . '.sql', 'w+');

 ...
 */
class Admin_PdfController extends Coringa_Controller_Alicerce_Action {
    ...
    public function contratoAction() {
        ...
        $dbc = new Admin_Model_DbTable_Cliente();
        $selc = $dbc->select();
        ...
        if ($retorno['cod_obra'] > 0) {
            $dbo = new Admin_Model_DbTable_Obra();
        ...
        foreach ($retel as $row) {
            if ($row['status_atual'] != 'C') {
                ...
                if ($row['ind_acessorio'] == 'S') {
                    $dba = new Admin_Model_DbTable_Acessorio();
...
//        $pdf->Cell(60, 6, number_format($total, 2, ',', '.'), 0, 1, "R");
        ...
        $pdf->mContrato(utf8_decode($contrato));
        $pdf->Output();
    }
...
// $fim identifica onde que deve se dar junção de centenas por "e" ou por "," ;)
    ...
    public function coringaContratoAction() {
        ...
        $dbe = new Admin_Model_DbTable_Empresa();
